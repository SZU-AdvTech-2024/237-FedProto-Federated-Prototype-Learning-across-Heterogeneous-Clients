...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        #self.dropout = nn.Dropout(p=0.2) ##my_add
...
class ResNet(nn.Module):
    ...
    def __init__(self, args, block, layers, num_classes=1000, zero_init_residual=False): 
        ...
        self.dropout = nn.Dropout(p=0.5) ##my_add
    ...
    def forward(self, x):
        ...
        if self.args.use_resnet_dropout:  ##
            x = self.dropout(x) ##my_add  ##self.eval()令self.training=False
...
def resnet18(args, pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(args.resnet_dir + model_names['resnet18']))  ##
    return model
