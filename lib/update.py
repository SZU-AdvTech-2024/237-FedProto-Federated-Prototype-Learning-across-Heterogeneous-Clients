...
class LocalUpdate(object):
    def __init__(self, args, dataset, idxs, priorList=None, classes_list=None):
        self.args = args
        self.trainloader = self.train_val_test(dataset, list(idxs))
        self.device = args.device

        if self.args.SSL:
            self.criterion = ssl.MPULoss(self.args.num_classes, self.args.pu_weight, priorList, classes_list).to(self.device)
        else:
    ...
    def update_weights(self, idx, model, global_round):
        ...
        if self.args.optimizer == 'sgd':
            if self.args.dataset[0:5] == 'cifar':  ##
                optimizer = torch.optim.SGD(model.parameters(), lr=self.args.lr,
                                            momentum=0.5, weight_decay=1e-4)  ##add_weight_decay
            else : ##
                optimizer = torch.optim.SGD(model.parameters(), lr=self.args.lr,
                                            momentum=0.5) 
        ...
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, labels_g) in enumerate(self.trainloader):
                ...
                if self.args.SSL == True: ##
                    loss, loss_pu, loss_nll = loss
                ...
                for i in range(len(labels)):  ##应对无标签处理过的数据
                    if labels[i] >= self.args.num_classes:
                        labels[i] -= self.args.num_classes
            ...
            if (len_batch_loss <= 0):
                len_batch_loss = float('nan')
            epoch_loss.append(sum(batch_loss)/len_batch_loss)
        ##
        len_epoch_loss = len(epoch_loss) ##
        if (len_batch_loss <= 0):
            len_batch_loss = float('nan')
        epochs_avg_loss = sum(epoch_loss) / len(epoch_loss)
        print('| Global Round : {} | User: {} | Loss: {:.4f} | Acc: {:.4f}'.format(
            global_round, idx, epochs_avg_loss, acc_val.item()))  ## my add

        ##return model.state_dict(), sum(epoch_loss) / len(epoch_loss), acc_val.item()
        return model.state_dict(), epochs_avg_loss, acc_val.item() ##
    ...
    def update_weights_het(self, args, idx, global_protos, model, global_round=round):
        ...
        if self.args.optimizer == 'sgd':
            ...
            optimizer = torch.optim.Adam(model.parameters(), lr=self.args.lr,
                                         weight_decay=1e-4)
        ####label_g=tensor([0, 0, 1, 0]) when round=0, idx=0 and randomseed=1234, of course local_bs=4
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, label_g) in enumerate(self.trainloader):
                ...
                model.zero_grad()
                log_probs, protos = model(images)  ##log_probs.shape=torch.Size([4, 10]), protos.shape=torch.Size([4, 50])
                loss1 = self.criterion(log_probs, labels)

                if self.args.SSL == True: ##
                    loss1, loss1_pu, loss1_nll = loss1
                ...
                if len(global_protos) == 0:
                    ...
                    i = 0
                    for label in labels:  ##所有批次依次计算
                        if label.item() in global_protos.keys():  ##注意不要让无标签进入
                ...
                for i in range(len(labels)):
                    if label_g[i].item() in agg_protos_label:
                        agg_protos_label[label_g[i].item()].append(protos[i,:])   ##收集对应local_bs个labels的更新后的protos
                    elif label_g[i].item() < args.num_classes:  ##防止无标签原型
                        agg_protos_label[label_g[i].item()] = [protos[i,:]]

                log_probs = log_probs[:, 0:args.num_classes]
                _, y_hat = log_probs.max(1)  ##_每行最大值，y_hat每行最大值的下标

                labels = labels.squeeze() ##
                for i in range(len(labels)):  ##应对无标签处理过的数据
                    if labels[i] >= args.num_classes:
                        labels[i] -= args.num_classes
                acc_val = torch.eq(y_hat, labels).float().mean()  ##这是本地client的每一轮最后个批次的acc然后被return 
            ...
            len_batch_loss = len(batch_loss['total']) ##
            if (len_batch_loss <= 0):
                len_batch_loss = float('nan')
            epoch_loss['total'].append(sum(batch_loss['total'])/len_batch_loss)
            epoch_loss['1'].append(sum(batch_loss['1']) / len_batch_loss)
            epoch_loss['2'].append(sum(batch_loss['2']) / len_batch_loss)
            
        ##
        len_epoch_loss = len(epoch_loss['total']) ##
        if (len_batch_loss <= 0):
            len_batch_loss = float('nan')
        
        epoch_loss['total'] = sum(epoch_loss['total']) / len_epoch_loss
        epoch_loss['1'] = sum(epoch_loss['1']) / len_epoch_loss
        epoch_loss['2'] = sum(epoch_loss['2']) / len_epoch_loss
        ##
        print('| Global Round : {} | User: {} | Loss: {:.4f} | Acc: {:.4f}'.format(
            global_round, idx, epoch_loss['total'], acc_val.item()))  ## my add
...
def test_inference_new_het_lt(args, local_model_list, test_dataset, classes_list, user_groups_gt, global_protos=[], my_flag=True):
    """ Returns the test accuracy and loss.
    """
    loss, total, correct = 0.0, 0.0, 0.0
    loss_mse = nn.MSELoss()
                ...
                for i in range(images.shape[0]):
                    for j in range(args.num_classes):
                        if j in global_protos.keys() and j in classes_list[idx]:
                            d = loss_mse(protos[i, :], global_protos[j][0])  ##一个image的proto和每个类对应global_proto的距离
                            dist[i, j] = d
...
def save_protos(args, local_model_list, test_dataset, user_groups_gt):
    ...
    y = np.array(y)
    d = np.array(d)
    np.save('./saved/protos/' + args.alg + '_protos.npy', x)
    np.save('./saved/protos/' + args.alg + '_labels.npy', y)
    np.save('./saved/protos/' + args.alg + '_idx.npy', d)
