...
def mnist_noniid(args, dataset, num_users, n_list, k_list):
    ...
    dict_users = {}
    idxs = np.arange(num_shards*num_imgs)
    labels = dataset.train_labels.numpy()  ##warnings.warn("train_labels has been renamed targets")
    # sort labels
    idxs_labels = np.vstack((idxs, labels))
    idxs_labels = idxs_labels[:, idxs_labels[1, :].argsort()]  ##.argsort()对上述一维数组执行排序操作，并返回原数组中元素升序排列后的索引。换句话说，这个返回值是一个索引数组，指明了排序后元素在原数组中的位置。
    idxs = idxs_labels[0, :]
    label_begin = {}
    cnt=0
    ...
    for i in range(num_users):
        ...
        for each_class in classes:
            ...
            user_data = np.concatenate((user_data, idxs[begin : begin+k]),axis=0)  ###对于一维数组拼接，axis的值不影响最后的结果
